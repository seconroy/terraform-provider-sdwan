...
type Ntp struct {
	...
	NtpServers          []NtpNtpServers     `tfsdk:"ntp_servers"`
	Authentication      []NtpAuthentication `tfsdk:"authentication"`
	TrustedKeys         types.Set           `tfsdk:"trusted_keys"`
	TrustedKeysVariable types.String        `tfsdk:"trusted_keys_variable"`
}

type NtpNtpServers struct {
...
type NtpAuthentication struct {
	Optional                       types.Bool   `tfsdk:"optional"`
	Md5AuthenticationKeyId         types.Int64  `tfsdk:"md5_authentication_key_id"`
	Md5AuthenticationKeyIdVariable types.String `tfsdk:"md5_authentication_key_id_variable"`
	AuthenticationValue            types.String `tfsdk:"authentication_value"`
	AuthenticationValueVariable    types.String `tfsdk:"authentication_value_variable"`
}

func (data Ntp) getModel() string {
	return "ntp"
}
...
func (data Ntp) toBody(ctx context.Context) string {
	...
	body, _ = sjson.Set(body, "templateType", "ntp")
	...
	for _, item := range data.NtpServers {
		itemBody := ""
		...
		if !item.HostnameIpAddressVariable.IsNull() {
			...
			itemBody, _ = sjson.Set(itemBody, "name."+"vipVariableName", item.HostnameIpAddressVariable.ValueString())
		} else if item.HostnameIpAddress.IsNull() {
		} else {
			itemBody, _ = sjson.Set(itemBody, "name."+"vipObjectType", "object")
			itemBody, _ = sjson.Set(itemBody, "name."+"vipType", "constant")
			itemBody, _ = sjson.Set(itemBody, "name."+"vipValue", item.HostnameIpAddress.ValueString())
		}
		...
		if !item.AuthenticationKeyVariable.IsNull() {
			...
			itemBody, _ = sjson.Set(itemBody, "key."+"vipVariableName", item.AuthenticationKeyVariable.ValueString())
			...
			itemBody, _ = sjson.Set(itemBody, "key."+"vipValue", item.AuthenticationKey.ValueInt64())
		}
	...
	for _, item := range data.Authentication {
		itemBody := ""
		itemAttributes := make([]string, 0)
		itemAttributes = append(itemAttributes, "number")

		if !item.Md5AuthenticationKeyIdVariable.IsNull() {
			itemBody, _ = sjson.Set(itemBody, "number."+"vipObjectType", "object")
			itemBody, _ = sjson.Set(itemBody, "number."+"vipType", "variableName")
			itemBody, _ = sjson.Set(itemBody, "number."+"vipVariableName", item.Md5AuthenticationKeyIdVariable.ValueString())
		} else if item.Md5AuthenticationKeyId.IsNull() {
		} else {
			itemBody, _ = sjson.Set(itemBody, "number."+"vipObjectType", "object")
			itemBody, _ = sjson.Set(itemBody, "number."+"vipType", "constant")
			itemBody, _ = sjson.Set(itemBody, "number."+"vipValue", item.Md5AuthenticationKeyId.ValueInt64())
		}
		itemAttributes = append(itemAttributes, "md5")

		if !item.AuthenticationValueVariable.IsNull() {
			itemBody, _ = sjson.Set(itemBody, "md5."+"vipObjectType", "object")
			itemBody, _ = sjson.Set(itemBody, "md5."+"vipType", "variableName")
			itemBody, _ = sjson.Set(itemBody, "md5."+"vipVariableName", item.AuthenticationValueVariable.ValueString())
		} else if item.AuthenticationValue.IsNull() {
		} else {
			itemBody, _ = sjson.Set(itemBody, "md5."+"vipObjectType", "object")
			itemBody, _ = sjson.Set(itemBody, "md5."+"vipType", "constant")
			itemBody, _ = sjson.Set(itemBody, "md5."+"vipValue", item.AuthenticationValue.ValueString())
		}
		if !item.Optional.IsNull() {
			itemBody, _ = sjson.Set(itemBody, "vipOptional", item.Optional.ValueBool())
			itemBody, _ = sjson.Set(itemBody, "priority-order", itemAttributes)
...
func (data *Ntp) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(path + "server.vipValue"); len(value.Array()) > 0 {
		data.NtpServers = make([]NtpNtpServers, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			...
			if cValue := v.Get("name.vipType"); cValue.Exists() {
				if cValue.String() == "variableName" {
					...
					item.HostnameIpAddressVariable = types.StringValue(cv.String())
					...
					item.HostnameIpAddress = types.StringValue(cv.String())
					item.HostnameIpAddressVariable = types.StringNull()
				}
			} else {
				item.HostnameIpAddress = types.StringNull()
			...
			if cValue := v.Get("key.vipType"); cValue.Exists() {
				if cValue.String() == "variableName" {
					...
					item.AuthenticationKeyVariable = types.StringValue(cv.String())
					...
					item.AuthenticationKey = types.Int64Value(cv.Int())
					item.AuthenticationKeyVariable = types.StringNull()
				}
			} else {
				item.AuthenticationKey = types.Int64Null()
			...
			data.NtpServers = append(data.NtpServers, item)
			return true
		})
	}
	if value := res.Get(path + "keys.authentication.vipValue"); len(value.Array()) > 0 {
		data.Authentication = make([]NtpAuthentication, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := NtpAuthentication{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
			...
			if cValue := v.Get("number.vipType"); cValue.Exists() {
				if cValue.String() == "variableName" {
					item.Md5AuthenticationKeyId = types.Int64Null()

					cv := v.Get("number.vipVariableName")
					item.Md5AuthenticationKeyIdVariable = types.StringValue(cv.String())

				} else if cValue.String() == "ignore" {
					item.Md5AuthenticationKeyId = types.Int64Null()
					item.Md5AuthenticationKeyIdVariable = types.StringNull()
				} else if cValue.String() == "constant" {
					cv := v.Get("number.vipValue")
					item.Md5AuthenticationKeyId = types.Int64Value(cv.Int())
					item.Md5AuthenticationKeyIdVariable = types.StringNull()
				}
			} else {
				item.Md5AuthenticationKeyId = types.Int64Null()
			...
			if cValue := v.Get("md5.vipType"); cValue.Exists() {
				if cValue.String() == "variableName" {
					item.AuthenticationValue = types.StringNull()

					cv := v.Get("md5.vipVariableName")
					item.AuthenticationValueVariable = types.StringValue(cv.String())

				} else if cValue.String() == "ignore" {
					item.AuthenticationValue = types.StringNull()
					item.AuthenticationValueVariable = types.StringNull()
				} else if cValue.String() == "constant" {
					cv := v.Get("md5.vipValue")
					item.AuthenticationValue = types.StringValue(cv.String())
					item.AuthenticationValueVariable = types.StringNull()
				}
			} else {
				item.AuthenticationValue = types.StringNull()
			...
			data.Authentication = append(data.Authentication, item)
			return true
		})
	}
...
func (data *Ntp) hasChanges(ctx context.Context, state *Ntp) bool {
	hasChanges := false
	if len(data.NtpServers) != len(state.NtpServers) {
		hasChanges = true
	} else {
		for i := range data.NtpServers {
			if !data.NtpServers[i].HostnameIpAddress.Equal(state.NtpServers[i].HostnameIpAddress) {
				hasChanges = true
			}
			if !data.NtpServers[i].AuthenticationKey.Equal(state.NtpServers[i].AuthenticationKey) {
				hasChanges = true
			}
			if !data.NtpServers[i].VpnId.Equal(state.NtpServers[i].VpnId) {
				hasChanges = true
			}
			if !data.NtpServers[i].Version.Equal(state.NtpServers[i].Version) {
				hasChanges = true
			}
			if !data.NtpServers[i].SourceInterface.Equal(state.NtpServers[i].SourceInterface) {
				hasChanges = true
			}
			if !data.NtpServers[i].Prefer.Equal(state.NtpServers[i].Prefer) {
				hasChanges = true
			}
		}
	}
	if len(data.Authentication) != len(state.Authentication) {
		hasChanges = true
	} else {
		for i := range data.Authentication {
			if !data.Authentication[i].Md5AuthenticationKeyId.Equal(state.Authentication[i].Md5AuthenticationKeyId) {
				hasChanges = true
			}
			if !data.Authentication[i].AuthenticationValue.Equal(state.Authentication[i].AuthenticationValue) {
				hasChanges = true
			}
		}
	}
