...
func TestAccDataSourceSdwanTransportWANVPNInterfaceEthernetFeatureAssociateIPv6TrackerGroupFeature(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceSdwanTransportWANVPNInterfaceEthernetFeatureAssociateIPv6TrackerGroupFeaturePrerequisitesConfig + testAccDataSourceSdwanTransportWANVPNInterfaceEthernetFeatureAssociateIPv6TrackerGroupFeatureConfig(),
...
const testAccDataSourceSdwanTransportWANVPNInterfaceEthernetFeatureAssociateIPv6TrackerGroupFeaturePrerequisitesConfig = `
...
func testAccDataSourceSdwanTransportWANVPNInterfaceEthernetFeatureAssociateIPv6TrackerGroupFeatureConfig() string {
	config := ""
	...
	config += `
		data "sdwan_transport_wan_vpn_interface_ethernet_feature_associate_ipv6_tracker_group_feature" "test" {
			feature_profile_id = sdwan_transport_feature_profile.test.id
			transport_wan_vpn_feature_id = sdwan_transport_wan_vpn_feature.test.id
			transport_wan_vpn_interface_ethernet_feature_id = sdwan_transport_wan_vpn_interface_ethernet_feature.test.id
			id = sdwan_transport_wan_vpn_interface_ethernet_feature_associate_ipv6_tracker_group_feature.test.id
		}
	`
	return config
}
