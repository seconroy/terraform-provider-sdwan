		...
		for ii := range data.Definitions[i].Entries {
			if !data.Definitions[i].Entries[ii].SiteListIds.IsNull() {
				...
				if countSiteListVersions > 0 {
					...
					data.Definitions[i].Entries[ii].SiteListVersions = types.ListValueMust(types.StringType, []attr.Value{})
				}
			}
			if !data.Definitions[i].Entries[ii].VpnListIds.IsNull() {
				...
				if countVpnListVersions > 0 {
					...
					data.Definitions[i].Entries[ii].VpnListVersions = types.ListValueMust(types.StringType, []attr.Value{})
				}
			}
			if !data.Definitions[i].Entries[ii].RegionListIds.IsNull() {
				...
				if countRegionListVersions > 0 {
					...
					data.Definitions[i].Entries[ii].RegionListVersions = types.ListValueMust(types.StringType, []attr.Value{})
				}
			}
		}
	}
